...
| Variables explicatives |     Minimum                  |     Moyenne                   |    Maximum                   |     Écart-type  | 
| :--------------------: |:----------------------------:| :----------------------------:| :---------------------------:| :--------------:|  
|         Bedrooms       |`r min(donnees$bedrooms)`     | `r mean(donnees$bedrooms)`    |`r max(donnees$bedrooms)`     |`r sd(donnees$bedrooms)`|   
|         Bathrooms      |`r min(donnees$bathrooms)`    |`r mean(donnees$bathrooms)`    |`r max(donnees$bathrooms)`    |`r sd(donnees$bathrooms)`|       
|         Sqft_living    |`r min(donnees$sqft_living)`  |`r mean(donnees$sqft_living)` |`r max(donnees$sqft_living)`  |`r sd(donnees$sqft_living)`|     
|         Sqft_lot       |`r min(donnees$sqft_lot)`     | `r mean(donnees$sqft_lot)`    |`r max(donnees$sqft_lot)`     | `r sd(donnees$sqft_lot)` |      
|         Floors         |`r min(donnees$floors)`       |`r mean(donnees$floors)`      |`r max(donnees$floors)`       |`r sd(donnees$floors)`|          
|         Waterfront     |`r min(donnees$waterfront)`   |`r mean(donnees$waterfront)`  |`r max(donnees$waterfront)`   |`r sd(donnees$waterfront)`|      
|         View           |`r min(donnees$view)`         |`r mean(donnees$view)`        |`r max(donnees$view)`         | `r sd(donnees$view)`  |         
|         Condition      |`r min(donnees$condition)`    |`r mean(donnees$condition)`   |`r max(donnees$condition)`    |`r sd(donnees$condition)`|   
|         Grade          |`r min(donnees$grade)`        |`r mean(donnees$grade)`       |`r max(donnees$grade)`        | `r sd(donnees$grade)`   |     
|         Sqft_above     |`r min(donnees$sqft_above)`   |`r mean(donnees$sqft_above)`  |`r max(donnees$sqft_above)`   | `r sd(donnees$sqft_above)`|     
|         Sqft_basement  |`r min(donnees$sqft_basement)`|`r mean(donnees$sqft_basement)`|`r max(donnees$sqft_basement)`|`r sd(donnees$sqft_basement)`|  
|         Yr_built       |`r min(donnees$yr_built)`     |`r mean(donnees$yr_built)`    |`r max(donnees$yr_built)`     | `r sd(donnees$yr_built)`    |   
|         Yr_renovated   |`r min(donnees$yr_renovated)` |`r mean(donnees$yr_renovated)`|`r max(donnees$yr_renovated)` | `r sd(donnees$yr_renovated)`|   
|         Lat            |`r min(donnees$lat)`          |`r mean(donnees$lat)`         | `r max(donnees$lat)`         | `r sd(donnees$lat)`         |    
|         Long           |`r min(donnees$long)`         |`r mean(donnees$long)`        | `r max(donnees$long)`        |`r sd(donnees$long)`      |      
|    Sqft_living15  |`r min(donnees$sqft_living15)`|`r mean(donnees$sqft_living15)`| `r max(donnees$sqft_living15)`| `r sd(donnees$sqft_living15)`|     
|         Sqft_lot15     |`r min(donnees$sqft_lot15)`   |`r mean(donnees$sqft_lot15)`  |`r max(donnees$sqft_lot15)`    | `r sd(donnees$sqft_lot15)`  |
...
date <- ggplot(donnees, aes(x=date)) + geom_density() + theme_bw()
...
year <- ggplot(donnees, aes(x=yr_built)) + geom_density() + theme_bw()
...
date <- ggplot(donnees, aes(x=date)) + geom_density() + 
  ...
  xlab("Moment de l'année") + ylab("Densité") +
  theme_bw() + #saisonalité dans la vente de maison
...
ggmap(map, extent = "device") + stat_summary_2d(data = donnees ,
  ...
  ggtitle("King County, USA") + labs(x = "Longitude", y = "Latitude") +
...
Lpri <- ggplot(donnees, aes(x=date, y=log(price))) + geom_point(alpha=0.4) + 
...
bed <- ggplot(donnees, aes(x=factor(bedrooms), y=log(price))) + geom_boxplot() + 
  ggtitle("Logarithme du prix de vente des maisons selon le nombre de chambres") +
  xlab("Nombre de chambres") + ylab("Logarithme du prix de vente") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))

bath <- ggplot(donnees, aes(x=factor(floor(bathrooms)), y=log(price))) + geom_boxplot() + 
  ggtitle("Logarithme du prix de vente des maisons selon le nombre de salles de bain complètes") +
  xlab("Nombre de salles de bain complètes") + ylab("Logarithme du prix de vente") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))

flo <- ggplot(donnees, aes(x=factor(floors), y=log(price))) + geom_boxplot() + 
  ggtitle("Logarithme du prix de vente des maisons selon le nombre d'e chambres'étages") +
