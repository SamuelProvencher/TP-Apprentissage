...
donnees$reno <- as.integer((donnees$yr_renovated!=0)*1)

#creation de la variable age
donnees$age <- as.integer(ifelse(as.numeric(annee) - donnees$yr_built >= 0, as.numeric(annee) - donnees$yr_built, 0)) #cap à 115
...
donnees$expensive_area <- sapply(1:nrow(donnees),
    function(i) as.integer(donnees$lat[i] >= ymin & donnees$lat[i] <= ymax & donnees$long[i] >= xmin & donnees$long[i] <= xmax))
```
...
À première vue, le jeu de données n'a aucun doublon. En effet, aucune observation n'est identique en tout point. Cependant, lorsque la variable *ID* est regardée seule, il est possible de trouver 177 identifiants de maison qui se répètent à plus d'une reprise. En approfondissant notre recherche sur ces cas, il a été possible de remarquer que les maisons sont en tout point identiques, sauf pour la date de vente et le prix de vente. L'hypothèse qui a été retenue ici est que des entrepreneurs ou des spéculateurs tentent de faire du profit en achetant une maison et en la revendant plus cher à une date ultérieure. Pour cette raison, il a été décidé de conserver ces 177 observations pour l'analyse.

La visualisation des données à l'étude a permis de déceler quelques erreurs. Tout d'abord, $10$ observations avaient un nombre de salles de bain égal à $0$. Étant donné qu'il est impossible d'avoir une maison sans salle de bain et que ces observations ne représentent qu'un faible pourcentage du jeu de données, il a été convenu de les supprimer. Après avoir enlevé ces $10$ observations, il a été remarqué que $6$ maisons comptaient $0$ chambre. Comme ces maisons en question contenaient toutes un terrain et qu'elles représentaient une faible proportion des données, il a été décidé de les enlever. En outre, une observation avait $33$ chambres. En se fiant à l'aire habitable de la maison ainsi qu'aux nombres de salles de bain de cette maison, il a été convenu que le nombre de chambres avait subi une erreur de frappe. Puisqu'il était impossible de déterminer avec certitude le véritable nombre de chambres de la maison, il a été décidé de simplement retirer cette observation du jeu du données. 

Comme nous avons un nombre considérable d'entrées dans la base de données (plus de 21000), les conséquences d'enlever 17 entrées de données fautives sont minimes. Si jamais le patron de non-réponse des données manquantes en question n'était pas MCAR, nous savons que le fait d'utiliser uniquement les cas complets comme nous avons fait amènera un certain biais. Nous avons convenu que ce biais serait minime et que nous pouvions l'ignorer. Il a été envisagé d'imputer stochastiquement les données manquantes pour éviter le biais, mais nous avons réalisé que les tests statistiques pour déterminer un patron de non-réponse ne sont pas concluants lorsqu'on a une proportion extrêmement faible de données manquantes (respectivement 10, 6 et 1 observations sur plus de 21000).
...
* Le numéro d'identification de la maison (*ID*) a été retiré.
...
* La variable *sqft_living* a été retirée, car il a été réalisé que cette variable était la somme directe des variables *sqft_basement* et *sqft_above*. Un problème évident de multicolinéarité était donc présent et il a été décidé de retirer complètement la variable *sqft_living*.
...
```{r echo=FALSE, warning=FALSE, message=FALSE,fig.pos='H', fig.height=3, fig.width=4, fig.align="center"}
library(gridExtra)
library(ggplot2)
...
```{r, echo=F, message=FALSE,fig.pos='H', fig.height=3, fig.width=6, fig.align="center"}
lat <- ggplot(donnees, aes(x=lat, y = log(price))) + 
  geom_point(alpha = 0.1) +
  ggtitle("Prix de vente des maisons selon leur latitude (log)") +
  xlab("Latitude") + ylab("Logarithme du prix de vente") +
  theme_bw() + #asymétrie
  theme(plot.title = element_text(hjust = 0.5, size = 7), axis.title = element_text(size=6))

long <- ggplot(donnees, aes(x=long, y = log(price))) + 
  geom_point(alpha = 0.1) + 
  ggtitle("Prix de vente des maisons selon leur longitude (log)") +
  xlab("Longitude") + ylab("Logarithme du prix de vente") +
  theme_bw() + #log pour variable réponse est mieux
  theme(plot.title = element_text(hjust = 0.5, size = 7), axis.title = element_text(size=6))

grid.arrange(lat, long, nrow = 1, ncol=2)
```
...
\newline
```{r echo=FALSE, warning=FALSE, message=FALSE,fig.pos='H', fig.height=6, fig.width=6, fig.align="center"}
library(gridExtra)
...
gra <- ggplot(donnees, aes(x=factor(grade), y=log(price))) + geom_boxplot() +
  ggtitle("Effet du grade (construction/conception) sur le prix de vente d'une maison") +
...
b <- ggplot(donnees, aes(x=log(sqft_lot), y=log(price))) + geom_point(alpha=0.4) +
  ggtitle("Prix de vente des maisons selon la superficie du terrain (log)") +
  xlab("Logarithme de la superficie du terrain (pieds carrés)") + ylab("Logarithme du prix de vente") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5, size = 6), axis.title = element_text(size=6))

c <- ggplot(donnees, aes(x=log(sqft_above), y=log(price))) + geom_point(alpha=0.4) +
  ggtitle("Prix de vente des maisons selon la superficie des étages supérieurs (log)") +
  xlab("Logarithme de la superficie des étages supérieurs (pieds carrés)") + ylab("Logarithme du prix de vente") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5, size = 6), axis.title = element_text(size=6))

d <- ggplot(donnees[!donnees$sqft_basement ==0,], aes(x=log(sqft_basement), y=log(price))) + geom_point(alpha=0.4) +
  ggtitle("Prix de vente des maisons selon la superficie du sous-sol (log)") +
  xlab("Logarithme de la superficie du sous-sol (pieds carrés") + ylab("Logarithme du prix de vente") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5, size = 6), axis.title = element_text(size=6))

e <- ggplot(donnees, aes(x=log(sqft_living15), y=log(price))) + geom_point(alpha=0.4) +
  ggtitle("Prix de vente des maisons selon la superficie habitable du voisinage (log)") +
  xlab("Logarithme de la superficie habitable du voisinage (pieds carrés)") + ylab("Logarithme du prix de vente") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5, size = 6), axis.title = element_text(size=6))

f <- ggplot(donnees, aes(x=log(sqft_lot15), y=log(price))) + geom_point(alpha=0.4) +
  ggtitle("Prix de vente des maisons selon la superficie du terrain du voisinage (log)") +
  xlab("Logarithme de la superficie du terrain du voisinage (pieds carrés)") + ylab("Logarithme du prix de vente") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5, size = 6), axis.title = element_text(size=6))
...
bli <- ggplot(donnees, aes(x=log(sqft_above), y=log(price), col = factor(floors))) + 
    geom_point(alpha=0.5) +
    ggtitle("Prix de vente selon sqft_above et floors") +
    xlab("Superficie habitable au-dessus du sol (Pieds carrés)") + ylab("Logarithme du prix de vente") +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5, size = 6), axis.title = element_text(size=6),
          legend.key.size = unit(0, 'lines'), legend.title = element_text(size = 5), legend.position = "bottom") +
...
grid.arrange(blo, bla, grid::nullGrob(), bli, grid::nullGrob(),
                        layout_matrix = matrix(c(1,1,2,2,3,4,4,5), byrow = TRUE, ncol = 4))
```
...
La deuxième variable qui a été créée est celle pour savoir si la maison a été rénovée ou non depuis sa construction. Elle a été nommée *reno*. De prime abord, il a été testé si cette nouvelle variable ne devait pas plutôt être catégorielle ordinale avec des catégories allant de *10 ans et moins* pour les maisons ayant eu une rénovation dans les $10$ années précédant leur vente, *10 ans et plus* pour les maisons ayant eu une rénovation il y a $10$ années ou plus et *Jamais rénové* pour les maisons n'ayant jamais été rénovées depuis leur construction. Par contre, après une analyse plus poussée, les catégories autres que celle *Jamais rénové* affichaient une moyenne et une médiane de prix de vente similaires. De plus, la proportion de maisons rénovées était faible (4 %). Pour ces deux raisons, il a été décidé que séparer les maisons rénovées en deux catégories semblait exagéré, c'est pourquoi au final, la variable *reno* prend uniquement comme valeurs $1$ si la maison a déjà été rénovée depuis sa construction ou $0$ si elle n'a jamais été rénovée.
...
```{r echo=FALSE, message=FALSE, fig.pos='H', fig.height=3, fig.width=9, fig.align="center", warning=FALSE}
xmin <-  -122.4
xmax <-  -122
ymin <-  47.5
ymax <-  47.72
...
Pour déterminer que la création de cette nouvelle variable est pertinente, examinons si la présence d'une maison dans la zone sélectionnée a effectivement une influence sur son prix:

```{r, warning=F, echo = F, fig.height=3, fig.align="center"}
ggplot(donnees, aes(x=log(sqft_above), y = log(price), col = factor(expensive_area))) + 
    geom_point(alpha=0.4) +
    scale_color_brewer(palette = "Set1") +
    labs(colour = "Zone urbaine") +
    ggtitle("Prix de vente en fonction de la superficie habitable au-dessus du sol et de la localisation") +
    xlab("Logarithme de la superficie habitable au-dessus du sol") + ylab("Logarithme du prix de vente") +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5, size = 8), axis.title = element_text(size=6), legend.position = "right")
```
...
```{r echo=FALSE, warning=F, message=FALSE,fig.pos='H', fig.height=3, fig.width=6, fig.align="center"}
library(gridExtra)
a <- ggplot(donnees, aes(x=age, y=log(price))) + geom_point(alpha=0.1) + 
...
library(knitr)
tableau <- acp$eig
colnames(tableau) <- c("Valeur propre", "Pourcentage de la variance", "Pourcentage cumulatif de la variance")
kable(tableau)
```
...
Enchaînons avec la troisième composante. Cette dernière est particulièrement influencée par les variables *sqft_lot*, *sqft_lot15*, *expensive_area* et *floors*. Une maison ayant une troisième composante de grande valeur possède donc un grand terrain, ses voisins ont également un grand terrain, elle n'est pas située dans la zone métropolitaine et (à une moindre mesure) possède peu d'étages. Selon toute logique, cette dimension représente donc l'état urbain ou rural des maisons. En effet, une maison avec une troisième composante élevée serait une maison en campagne qui possède un très grand terrain et peu d'étages (les maisons de campagne sont souvent plus anciennes et possèdent moins d'étages que les maisons récentes). À l'inverse, une maison située en ville et possédant un petit terrain et beaucoup d'étages aura une valeur très faible pour la troisième composante.
...
![4ème composante sur la carte de King County](Composante_4_map.png){width="100%"}
...
* Si une analyse des variables que nous allons utiliser semble dévoiler un problème de multicolinéarité, la régression régularisée ridge pourrait être intéressante.
