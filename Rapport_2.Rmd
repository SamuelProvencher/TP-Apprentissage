...
source("4-Arbre.R")
```
Un modèle qui a été décidé de tester est un arbre de décision, mais plus précisément dans le cas présent, un arbre de régression. Pour ce faire, l'algorithme \textit{classification and regression tree} (CART) implanté dans le paquetage \hyperlink{target1}{{\color{blue}\textbf{{rpart}}}} a été utilisé. 
...
hist2 <- ggplot(mapping = aes(x = log(donnees.test$price))) +
  ...
  xlim(11, 16) +
  ylim(0, 750) +
  labs(title = "Graphique 12 : Histogrammes \ndes logarithmes de prix \nréels", y = "Fréquence", 
       x = "Log(prix réel)")
