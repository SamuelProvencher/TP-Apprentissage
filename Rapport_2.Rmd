...
source("3-K_voisin.R")
```
Un autre modèle qui a été testé est celui des *k* plus proches voisins. Étant donné que nous sommes en présence d'un problème de régression, la fonction `knn.reg` du paquetage \hyperlink{target1}{{\color{blue}\textbf{FNN}}} a été utilisée pour construire ce modèle. 

Le modèle des *k* plus proches voisins est simple. Afin d'effectuer une prédiction pour une observation dont les valeurs des variables explicatives sont comprises dans le vecteur $\textbf{x}_0$, il faut regarder l'ensemble des *k* plus proches voisins de $\textbf{x}_0$, c'est-à-dire les *k* observations qui en sont le plus près selon la distance euclidienne. Puis, la prévision du point $\textbf{x}_0$ est la moyenne des variables réponses des observations comprises dans l'ensemble des *k* plus proches voisins.

Il faut d'abord déterminer la valeur optimale de *k*, soit le nombre de voisins à considérer. Pour ce faire, une validation croisée à dix plis a été utilisée. La fonction `train` du paquetage \hyperlink{target1}{{\color{blue}\textbf{{caret}}}} a été utilisée pour faire cette validation croisée. Étant donné que le modèle des *k* plus proches voisins est fondé sur la distance euclidienne, les données ont été standardisées avant de procéder à la validation croisée. La métrique choisie pour sélectionner la valeur de *k* est l'erreur quadratique moyenne (`metric="RMSE"`). Ainsi, la valeur de *k* qui minimisait l'erreur quadratique moyenne est $k=9$, tel qu'illustré sur le graphique 1.
...
plot(1:length(bag$mse),bag$mse, type="l", xlab="Nombre d'arbres composant le bagging", ylab="EQM des OOB", main="Graphique 2 : EQM des observations OOB \nen fonction du nombre d'arbres")
```
...
Un autre modèle qui a été testé est celui de la forêt aléatoire. Pour ce faire, le même algorithme \textit{classification and regression with Random Forest (randomForest)} implanté dans le paquetage \hyperlink{target1}{{\color{blue}\textbf{{randomForest}}}} a été utilisé.

Le *bagging* et la forêt aléatoire sont très similaires, mais la forêt aléatoire permet de décorreler les arbres à l'aide de deux méthodes.
...
title(main="Graphique 4: EQM selon le nombre d'itérations (d=7)", 
...
Selon ce critère, le meilleur modèle est celui obtenu avec $d=9$ et `r n.iter9` itérations. C'est donc ce modèle que nous utiliserons lors de la comparaison des différents modèles entre eux. 
...
varImpPlot(foret, main="Graphique 5 : Importance des variables composant la forêt aléatoire")
```
...
\subsubsection{Analyse des résultats obtenus avec le modèle}
...
```{r histresults, echo=F, message=F, warning=F}
library(gridExtra)
hist1 <- ggplot(mapping = aes(x = PredGbm)) +
  ...
  xlim(11, 16) +
  ylim(0, 750) +
  labs(title = "Graphique 11 : Histogramme \ndes logarithmes de prix prédits \npar le GBM", y = "Fréquence", 
       x = "Log(prix prédit)")
...
fun <- summary(gbm.modele9, n.trees = n.iter9, las = 1, cex.names = 0.5, main = "Graphique 13 : Importance des variables dans le GBM")
```
...
```{r gbmpdp_grade, echo=F,fig.pos='H', fig.height=4, fig.width=5, fig.align="center"}
plot(gbm.modele9, n.trees = n.iter9, i.var = "grade", main ="Graphique 14 : graphique de dépendance partielle \n de la variable grade")
```
...
```{r gbmpdp_lat, echo=F,fig.pos='H', fig.height=4, fig.width=5, fig.align="center"}
plot(gbm.modele9, n.trees = n.iter9, i.var = "lat", main ="Graphique 15 : graphique de dépendance partielle \n de la variable lat")
```
...
```{r gbmpdp_sqftabove, echo=F,fig.pos='H', fig.height=4, fig.width=5, fig.align="center"}
plot(gbm.modele9, n.trees = n.iter9, i.var = "sqft_above", main = "Graphique 16 : graphique de dépendance partielle \n de la variable sqft_above")
```
...
ggplot(hstat.grade, aes(x = Variables, y = HStat)) + 
  ...
  coord_flip() +
  theme_bw() +
  labs(title = "Graphique 17 : graphique des statistiques H \n selon la variable grade") +
  ylab("Statistiques H") +
  xlab("Interaction des variables avec grade") +
  theme(plot.title = element_text(size = rel(1.4), h=0.5))
```
...
plot(gbm.modele9, i.var = c("grade", "lat"), n.trees = n.iter9, main = "Graphique 18 : Graphique de dépendance \npartielle bivarié entre les variables grade et lat")
```
...
plot(gbm.modele9, i.var = c("long", "lat"), n.trees = n.iter9, main = "Graphique 20 : Graphique de dépendance \npartielle bivarié entre les variables long et lat")
```
