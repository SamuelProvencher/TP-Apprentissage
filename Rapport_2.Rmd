...
Si on permute de façon aléatoire les valeurs de la variable à analyser voulue, on calcule de nouvelles prévisions sur nos données et ensuite on compare la nouvelle erreur quadratique moyenne avec celle du jeu de données initial, on obtient la *mean decrease in accuracy*. Pour notre jeu de données, on voit donc que les variables de la *latitude*(lat) et la *longitude*(long) sont celles qui ont le plus d'impact sur nos prévisions si on applique cette permutation. Suit juste après de la superficie du terrain en pieds carrés. En effet, autant la superficie du terrain de la maison ainsi que celle moyenne des 15 autres maisons autour on une bonne importance sur les prévisions du logarithme de prix d'achat de la maison.

Si on calcule la diminution totale dans l'erreur quadratique moyenne due à une séparation sur la variable à analyser voulue, on obtient la *mean decrease in node impurity*. Avec cette mesure, ce n'est pas les mêmes variables qui sont dites importantes pour notre modèle. Non, en fait, hormis la latitude qui figure toujours dans le top 4 des variables les plus importantes, la qualité de la construction et de la conception (grade) apparait comme étant celle qui se démarque le plus. La superficie habitable moyenne des 15 plus proches voisins (sqft_living15) et la superficie habitable du sous-sol de la maison sont celles qui viennent compléter le top 4.
