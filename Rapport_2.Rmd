...
Le modèle des *k* plus proches voisins est simpliste. Afin de prédire la valeur d'un point $x_0$, il faut regarder l'ensemble des *k* plus proche voisins de $x_0$, c'est-à-dire les observations qui minimisent la distance Euclidienne. Puis, la prévision du point $x_0$ est la moyenne de la valeur des observations comprises dans l'ensemble des *k* plus proches voisins.

Il faut d'abord déterminer la valeur optimale de *k*, soit le nombre de voisins à considérer. Pour se faire, une validation croisée à 10 plis a été utlisée. La fonction `train` du paquetage \hyperlink{target1}{{\color{blue}\textbf{{caret}}}} a été utilisée pour faire cette validation croisée. Étant donnée que le modèle des *k* plus proches voisins est fondé sur la distance Euclidienne, les données ont été standardisées avant de procéder à la validation croisée. La métrique sélectionnée pour sélectionner la valeur de *k* est l'erreur quadratique moyenne. Ainsi, la valeur de *k* qui minisait l'erreur quadratique moyenne est $k=9$. 
...
source("4-Arbre.R")
```
Un modèle qui a été décidé de tester est un arbre de décision. Pour ce faire, l'algorithme \textit{classification and regression tree} (CART) implanté dans le paquetage \hyperlink{target1}{{\color{blue}\textbf{{rpart}}}} a été utilisé. 
